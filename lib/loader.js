...
// Every redemption POSTs to /api/redeem: the server validates the key and
// returns the packed payload + seed, the snippet regenerates the
...
function buildLoader(baseUrl, key) {
	...
	const [
		...
		parsed, buf, src, fn, ok, good, ui, gateRun,
	] = ids(16);
...
// same Park-Miller + XOR cipher and the packed blob rides inside the snippet,
// so it needs no server and no key. This is the site's "convert to loadstring"
// feature. `seed` selects the keystream so two builds are never identical.
...
module.exports = { buildLoader, buildLoadstring };