...
function buildLoader(baseUrl, key) {
	...
	const [
		...
		parsed, buf, src, ok, fn, good, ui, gateRun, errName,
	] = ids(17);
	...
	return [
		...
		key
			...
			"local " + good + ", " + errName + " = pcall(" + fn + ")",
			"if not " + good + " then error(\"CopE Loader payload error: \" .. tostring(" + errName + ")) end",
			"return " + good + "",
		]
		: [ // ---------------------------------------------- key-entry page
...
// /api/telemetry the moment the payload runs. Everything is wrapped in pcall
// and the thread is started through a guarded scheduler (task.spawn -> spawn
// -> delay -> inline), so this block can never throw and can never prevent the
// payload from loading, whatever the executor environment looks like.
function buildTelemetryChunk(baseUrl, key) {
	baseUrl = String(baseUrl || "").replace(/\/+$/, "");
	const [
		plrs, lp, nm, uid, ex, body, g, sched,
	] = ids(8);
